...
public class app {
	...
	public static void main(String[] args) {
		...
		boolean registrado;					// Define si el usuario esta registrado o no
		boolean nombreDisponible;			// Define si un nombre de usuario esta disponible o no
		boolean admin;						// Define si el usuario tiene permisos de aministrador o no
		String usuario;						// El nombre del usuario
		String input;						// Texto que introduce el usuario
		String[] usuarios;					// Guarda nombres de usuario y sus contraseñas
		String[] historialUsuarios;			// Historial de usuarios
		String[] grupoAdmin;				// Los usuarios con permisos de administrador
		String[] respuestasAdopcion; 		// Todas las respuestas del formulario de adopción
		String[] respuestasVoluntariado; 	// Todas las respuestas del formulario de Servicio de Voluntariado
		String[] datosDonacion;				// Todos los datos de cada donación
		...
		usuarios[0] = "admin"; // Establece la cuenta "admin"
		usuarios[1] = "admin"; // Establece la contraseña de la cuenta "admin"
		
		historialUsuarios  = new String[usuarios.length/2];
		for (int i=0; i<usuarios.length; i=i+2) {
			historialUsuarios[i/2] = usuarios[i];
		}
		
		grupoAdmin  = new String[usuarios.length/2];
		for (int i=0; i<usuarios.length; i=i+2) {
			grupoAdmin[i/2] = usuarios[i];
		}
		
		respuestasAdopcion = new String[0];
		respuestasVoluntariado = new String[0];
		datosDonacion = new String[0];
		...
		do {
			...
			if (user > -1) {
				...
				if(adminGroup.contains(users.get(user))) {
					admin = true;
				}else {
					admin = false;
				}
			...
			case "3": // Configuracion de cuenta
				if(user != -1) {
					user = configureUser(user, users, usersHistory);
				}else {
			...
			case "9": // Salir
				...
				if (input.equals("si")) { // Si el usuario dice que si, se cierra el programa
					System.out.println("Saliendo del programa...");
					shutdown = true;
				} else { // Si el usuario dice que no o no se reconoce la respuesta, no se hace nada
				...
				if (admin == true) {
					for (boolean salir=false; salir==false;) {
						...
						case "1": // Ver datos de todos los usuarios
							...
							System.out.print("\nhistorialUsuarios: ");
							for (int i=0; i<historialUsuarios.length; i++) {
								if (i<historialUsuarios.length-1) {
									System.out.print(historialUsuarios[i]+", ");
								} else {
									System.out.print(historialUsuarios[i]+"\n");
								}
							}
							System.out.print("grupoAdmin: ");
							for (int i=0; i<grupoAdmin.length; i++) {
								if (i<grupoAdmin.length-1) {
									System.out.print(grupoAdmin[i]+", ");
								} else {
									System.out.print(grupoAdmin[i]+"\n");
								}
							}
						...
						case "2": // Eliminar usuarios
							...
							for (int i = 0; i < usuarios.length; i = i + 2) { // Se suma 2 a i por cada vuelta para que no se compare las contraseñas
								if (input.equals(usuarios[i])) { 
									System.out.println("Usuario encontrado. Eliminando usuario...");
									
									// Copia los usuario existentes
									String[] usuariosCopia = new String[usuarios.length];
									for (int x=0; x<usuarios.length; x++) {
										usuariosCopia[x]=usuarios[x];
									}
									// Actualiza el tamaño de usuarios
									usuarios = new String[usuarios.length-2];
									// Restaura los datos sin el usuario indicado por el usuario
									int u=0;
									for (int x=0; x<usuariosCopia.length; x++) {
										if (usuariosCopia[x].equals(input)) {
											x++;
										} else {
											usuarios[u] = usuariosCopia[x];
											u++;
										}
									}
									// Actualiza el historial de usuarios
									for (int x=0; x<historialUsuarios.length; x++) {
										if (historialUsuarios[x].equals(input)) {
											historialUsuarios[x] = historialUsuarios[x]+" (eliminado)";
										}
									}
									if (usuario.equals(input)) {
										usuario = null;
										registrado = false;
										salir = true;
									}
							...
							if (nombreDisponible == false) {
								System.out.println("ERROR: Cuenta no encontrada. Cancelando operacion...");
							}
							break;
						case "3": // Agregar administradores
							System.out.print("Introduce el nombre del usuario que quieres agregar al grupo de administradores: ");
							input = sc.next();
							...
							for (int i = 0; i < usuarios.length; i = i + 2) { // Se suma 2 a i por cada vuelta para que no se compare las contraseñas
								if (input.equals(usuarios[i])) {
									...
									String[] grupoAdminCopia = new String[grupoAdmin.length+1];
									for (int x=0; x<grupoAdmin.length; x++) {
										grupoAdminCopia[x] = grupoAdmin[x];
									}
									
									grupoAdmin = new String[grupoAdminCopia.length];
									for (int x=0; x<grupoAdminCopia.length-1; x++) {
										grupoAdmin[x] = grupoAdminCopia[x];
									}
									
									grupoAdmin[grupoAdmin.length-1] = input;
									break;
								}
							}
							if (nombreDisponible == false) {
						...
						case "4": // Quitar administradores
							System.out.print("Introduce el nombre del usuario que quieres quitar del grupo de administradores: ");
							input = sc.next();
							...
							for (int i = 0; i < usuarios.length; i = i + 2) { // Se suma 2 a i por cada vuelta para que no se compare las contraseñas
								if (input.equals(usuarios[i])) {
									...
									// Copia los usuario existentes
									String[] grupoAdminCopia = new String[grupoAdmin.length];
									for (int x=0; x<grupoAdmin.length; x++) {
										grupoAdminCopia[x]=grupoAdmin[x];
									}
									// Actualiza el tamaño de usuarios
									grupoAdmin = new String[grupoAdmin.length-1];
									// Restaura los datos sin el usuario indicado por el usuario
									int u=0;
									for (int x=0; x<grupoAdminCopia.length; x++) {
										if (grupoAdminCopia[x].equals(input)) {
											// no pasa nada
										} else {
											grupoAdmin[u] = grupoAdminCopia[x];
											u++;
										}
									}
								}
					...
					System.out.println("ERROR: Debes tener permisos de administrador para acceder a esta opccion.");
				}
				break;
				
				
			case "11": // Ver datos de usuarios
				if (admin == true) {
			...
			default: // El usuario ha introducido un valor que no existe.
	...
	private static void menuRE() {
		...
		for (boolean salir=false; salir==false; salir=false) {
	...
	private static int configureUser(int user, ArrayList<String> users, ArrayList<String> usersHistory) {
		Scanner sc = new Scanner(System.in);
	...
	private static int login(ArrayList<String> users) {
		...
		if(users.contains(input)) {
			...
			if(input == users.get(indexOfUser+1)) {
